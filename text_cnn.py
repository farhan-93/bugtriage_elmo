...
class TextCNN(object):
	def __init__(self, sequence_length, num_classes, vocab_size, embedding_size, filter_sizes, num_filters, l2_reg_lambda=0.1):
		...
		with tf.device('/cpu:0'), tf.name_scope('embedding'):
			...
			initW = embed(tf.squeeze(tf.strings.as_string(self.input_x)), signature="default", as_dict=True)['elmo']


			initW=tf.squeeze(initW,0)
			initW = initW[:,:300]
		...
		for i, filter_size in enumerate(filter_sizes):
			with tf.name_scope('conv-maxpool-%s' % filter_size):
				...
				conv = tf.nn.conv2d(
					...
					name='conv')
				# Apply nonlinearity
				h = tf.nn.leaky_relu(tf.nn.bias_add(conv, b), alpha=0.2, name='relu')
